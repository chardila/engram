...
│   ├── plugin/                      ← skills y CLAUDE.md para Claude Code
...
├── MCP "brain-work"     → apunta al vault de trabajo
└── MCP "brain-personal" → apunta al vault personal
...
├── plugin/                      ← fuente de verdad de los skills
│   ├── CLAUDE.md                ← instrucciones base para Claude (referencia el vault)
│   └── skills/
│       ├── daily-prep.md        ← /daily-prep
│       ├── end-day.md           ← /end-day
│       ├── process-inbox.md     ← /process-inbox
│       ├── review-week.md       ← /review-week
│       ├── analyze-vault.md     ← /analyze-vault
│       └── review-system.md     ← /review-system
└── vault-template/              ← template del vault con ejemplos ficticios comentados
    ...
    ├── Skills/                  ← copias de plugin/skills/ — setup.sh las copia aquí
    ├── Inbox/
    ├── Projects/dev/
    ├── Areas/
    ├── Resources/concepts/
...
> **Convención:** `plugin/skills/` es la fuente de verdad. `vault-template/Skills/` son copias generadas por `setup.sh`. Esto permite actualizar los skills vía `claude plugin update` sin tocar el vault personal.
...
if git ls-remote "$BRAIN_REPO" &>/dev/null; then
  ...
  # Copiar skills (fuente de verdad: plugin/skills/)
...
echo "   Agrega manualmente a $MCP_CONFIG (usando el nombre del vault como clave MCP):"
echo '   "mcpServers": { "'"$VAULT_NAME"'": { "command": "npx", "args": ["-y", "@modelcontextprotocol/server-filesystem", "'"$VAULT_PATH"'"] } }'
...
> **Nota:** El script usa `@modelcontextprotocol/server-filesystem` por defecto — más robusto que `mcp-obsidian` ya que no requiere Obsidian corriendo. Ver Paso 7 para la alternativa con `mcp-obsidian`.
...
3. Solo tras aprobación explícita: editar el archivo correspondiente en plugin/skills/ o CLAUDE.md
...
Cada instancia de vault tiene su propia entrada en `mcpServers`. La clave es el nombre del vault — así Claude sabe a cuál vault está apuntando en cada sesión.

### Configuración en Mac (Claude Desktop)
  ...
  "mcpServers": {
    "brain-work": {
      ...
      "args": ["-y", "@modelcontextprotocol/server-filesystem",
               "/Users/<tu-usuario>/vault/brain-work"]
    },
    "brain-personal": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-filesystem",
               "/Users/<tu-usuario>/vault/brain-personal"]
    }
  }
}
```

### Configuración en Linux (Claude Code)

Editar: `~/.claude/settings.json`, agregar dentro del objeto principal:
...
```json
{
  "mcpServers": {
    "brain-work": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-filesystem",
               "/home/<tu-usuario>/vault/brain-work"]
    },
    "brain-personal": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-filesystem",
               "/home/<tu-usuario>/vault/brain-personal"]
    }
  }
}
```

> **Nota:** Ambas instancias están disponibles en la misma sesión de Claude, pero los skills operan sobre una sola a la vez. Al ejecutar `/daily-prep`, Claude usa el vault que corresponde al contexto activo — esto se especifica en el `CLAUDE.md` de cada vault. Si tienes ambas activas, indica al inicio de la sesión cuál usar: *"sesión de trabajo"* o *"sesión personal"*.

> **Alternativa con mcp-obsidian:** Si prefieres `mcp-obsidian` (requiere Obsidian corriendo con el plugin "Local REST API"), reemplaza `@modelcontextprotocol/server-filesystem` por `mcp-obsidian` y agrega `"env": { "OBSIDIAN_VAULT_PATH": "<ruta>" }`. Verificar que sigue mantenido antes de instalar: https://github.com/calclavia/mcp-obsidian
...
- **Skills: fuente de verdad en `plugin/skills/`**. Los archivos en `vault-template/Skills/` son copias que `setup.sh` genera al crear el vault. Si editas un skill, edítalo en `plugin/skills/` y vuelve a copiar — o actualiza el plugin con `claude plugin update`.
